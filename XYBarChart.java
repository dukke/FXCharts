...
//    }
    ...
    @Override protected void layoutPlotChildren() {
        ...
        int seriesSize =(int) ReflectionUtils.forceMethodCall(XYChart.class, "getSeriesSize", this);
        ...
        if (getData() != null) {
            for (int seriesIndex=0; seriesIndex < getData().size(); seriesIndex++) {
                ...
                legenditem.getSymbol().getStyleClass().addAll("chart-bar","series"+seriesIndex,"bar-legend-symbol"
                        /*,series.defaultColorStyleClass*/, (String) ReflectionUtils.forceFieldCall(Series.class, "defaultColorStyleClass", series));
...
//        if (seriesCategoryMap.isEmpty() && categoryAxis.isAutoRanging()) categoryAxis.getCategories().clear();
    ...
    private void animateDataAdd(Data<X,Y> item, Node bar) {
        ...
        if (orientation == Orientation.VERTICAL) {
            ...
            ReflectionUtils.forceMethodCall(Data.class, "setCurrentY", item, getYAxis().toRealValue((barVal < 0) ? -bottomPos : bottomPos));
            getPlotChildren().add(bar);
...
//            );
            ...
            ReflectionUtils.forceMethodCall(Data.class, "setCurrentX", item, getXAxis().toRealValue((barVal < 0) ? -bottomPos : bottomPos));
            getPlotChildren().add(bar);
